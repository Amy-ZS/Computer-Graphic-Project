...
We have a fluid simulation going (that is our physics pillar). It is able to run 20k+ particles at 60+ fps. The build has gotten a little less optimized since our MVP but that is because we are trying to fix our collisions.

## Second Pillar Integration

Our second pillar, which is the rendering, has already been implemented. We still only have the render for the fluids but we are close to getting one for the world. We just have a ray tracer that runs when you press F.

We are also in the process of making our UI look better by using Godot's built in components so that you can access the image generated.

## New features:

Visible collision boxes that can be toggled.
Fixed our SDF mesh seeing objects it should not have seen
Better UI
